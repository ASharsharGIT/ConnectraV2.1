...
AnimationController.initHoverEffects = function() {
    const reduced = document.body.classList.contains('reduced-motion') || document.body.classList.contains('battery-save');

    // 3D Tilt Effect for Cards (skip if reduced motion / battery save)
    if (!reduced) {
        ...
        tiltCards.forEach(card => {
            ...
            card.addEventListener('mousemove', (e) => this.updateTiltEffect(card, e));
        });
    }

    // Magnetic Effect for Buttons (softened when reduced)
    const magneticButtons = document.querySelectorAll('.cta-button');
    magneticButtons.forEach(button => {
        button.addEventListener('mouseenter', () => {
            if (!reduced) this.addMagneticEffect(button);
        });
        button.addEventListener('mouseleave', () => this.removeMagneticEffect(button));
        button.addEventListener('mousemove', (e) => {
            if (!reduced) this.updateMagneticEffect(button, e);
        });
    });
};
